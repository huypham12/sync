...
int crypto_init(const char *key_file_path) {
  ...
  if (read_bytes < AES_KEY_SIZE) {
    fprintf(stderr,
            "Warning: Key file is smaller than %d bytes. Padding with 0s.\n",
            AES_KEY_SIZE);
...
/*
Sự liên hệ giữa 2 file này là ví dụ điển hình của mô hình Người làm - Kẻ ghi
chép:

crypto.c (Người làm): Nó như một cái cân điện tử. Đưa đồ vật (file) cho nó, nó
sẽ trả về con số chính xác. Nhiệm vụ của nó dừng lại tại đó, nó không nhớ kết
quả, cũng không ghi chép lại. index_manager.c (Kẻ ghi chép): Nó là một thủ thư
cần mẫn, có một cuốn sổ tay (.sync_state.csv). Nó không biết cân đo đong đếm, ai
đọc cho nó con số nào thì nó chép vào sổ con số đó. Kịch bản 1: Khi Daemon khởi
động (Phát hiện offline changes)

Daemon quét danh sách file trong máy tính.
Với mỗi file quét được, Daemon gọi compute_sha256 của crypto.c. crypto nhả ra
chuỗi băm (VD: abc123...). Daemon vác chuỗi abc123... đó qua index_manager.c,
gọi hàm index_get() để hỏi: "Ê thủ thư, trong sổ của ông cái file này lần trước
có mã băm là bao nhiêu?". Nếu thủ thư nói trong sổ là xyz999... $\rightarrow$
Daemon biết ngay file đã bị người dùng sửa trong lúc app tắt, lập tức mang đi
đồng bộ! Kịch bản 2: Khi đồng bộ xong (Cập nhật sổ bộ)

Khi file mới từ trên mạng được tải về máy thành công.
Trình nhận (receiver) sẽ gọi compute_sha256 (trong crypto.c) tính ra mã băm mới
nhất. Trình nhận đưa mã băm mới nhất này cho index_manager.c thông qua lệnh
index_update(). index_manager dùng bút tẩy (xóa băm cũ đi) và chép băm mới vào
danh sách liên kết của nó, đảm bảo hệ thống nhớ được hình dáng mới nhất của
file.
*/