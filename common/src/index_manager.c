...
void index_init(const char *sync_dir) {
  ...
  // Set file path
  snprintf(state_file_path, sizeof(state_file_path), "%s/.sync_state.csv",
           sync_dir);
...
void index_save(void) {
  ...
  while (curr) {
    fprintf(f, "%s,%llu,%s\n", curr->file_name,
            (unsigned long long)curr->file_size, curr->checksum);
    curr = curr->next;
  }
...
/*
cái này dùng để quét việc đồng bộ các file thay đổi lúc offline
